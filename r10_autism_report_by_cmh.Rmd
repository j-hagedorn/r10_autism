...
### This report was generated for `r paste0(params$cmh_wsa)` on `r Sys.Date()`
...
svs <- read_csv(paste0(csv_path,"svs.csv"))
...
svs <-
svs %>%
  # Filter to limit to CMHSP
  filter(
    PROVIDER_NAME == params$cmh
  ) %>%
...
```{r}
wsa <- read_csv(paste0(csv_path,"wsa.csv"))

wsa %<>%
  filter(
    PIHP_CMH_Name == params$cmh
  ) %>%
  mutate(
...
wsa_ipos <- read_csv(paste0(csv_path,"wsa_ipos.csv"))
