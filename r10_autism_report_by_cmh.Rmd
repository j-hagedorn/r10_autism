...
wsa_tbl %>%
  datatable(
    rownames = FALSE,
    colnames = c('Organization','Total Enrollees','Overdue Enrollees','% of Enrollees Overdue'),
    extensions = c('Responsive','Buttons'),
    options = list(
      dom = 't',
      buttons = c('colvis')
...
wsa %>%
  ...
  mutate(
    ...
    days_wo_bpos = as.numeric(max(svs$FROM_DATE, na.rm = T) - as.Date(Eligibility_Date))
  ) %>%
...
waitlist_summary %>%
  datatable(
    rownames = FALSE,
    colnames = c('Organization','Total Enrollees without a Behavioral Plan of Service','Enrollees Without Any Services','% Without Services'),
    extensions = c('Responsive','Buttons'),
    options = list(
      dom = 't',
      buttons = c('colvis')
