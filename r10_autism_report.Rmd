...
<h4>This *full detail* report was generated for Region 10 PIHP on `r Sys.Date()` using data from `r min(svs$FROM_DATE)` - `r max(svs$FROM_DATE)`</h4>
...
wsa %>%
  ...
  mutate(
    ...
    days_wo_bpos = as.numeric(max(svs$FROM_DATE) - as.Date(Eligibility_Date))
  ) %>%
  datatable(
    rownames = FALSE,
...
dt <- t3 %>%
  ...
  datatable(
    ...
    colnames = c('WSA ID','Encounters per month',
                 ...
                 'Days without BPOS','Percent days covered',
                 'Total Encounters'),
    extensions = c('Responsive','Buttons'),
    options = list(
      columnDefs = cd,
