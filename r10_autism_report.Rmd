...
The table below shows the number of individuals who received autism benefit services during the last 90 days, and the number of those individuals who did not receive family training services during that time.
...
waitlist_svs <-
  ...
  mutate(last_90_log = FROM_DATE >= last_90) %>%
  group_by(Case_ID,PIHP_CMH_Name,CPT_CD,SERVICE_DESC,UNIT_TYPE,AUTISM_SRV,Eligibility_Date,last_90_log) %>%
...
waitlist_last90 <-
  waitlist_svs %>%
  # Only include services if they are in most recent 90 days of dataset
  filter(last_90_log == T) %>%
  ungroup() %>%
  select(-PIHP_CMH_Name)
...
waitlist_summary <-
  ...
  left_join(waitlist_last90, by = c("Case_ID")) %>%
  group_by(Case_ID,PIHP_CMH_Name) %>%
...
waitlist_person <-
  ...
  filter(IPOSExists == F) %>%
  select(ID,Case_ID,PIHP_CMH_Name,Referral_Date:Eligibility_End_Date) %>%
  left_join(waitlist_svs, by = c("Case_ID")) %>%
  rename(
    PIHP_CMH_Name = PIHP_CMH_Name.x,
    Eligibility_Date = Eligibility_Date.x
  ) %>%
  select(-PIHP_CMH_Name.y, -Eligibility_Date.y) %>%
  group_by(Case_ID,PIHP_CMH_Name) %>%
  summarize(
    first = min(first),
    last = max(last),
