...
waitlist_person %>%
  select(Case_ID,svs_list,elig_interval,svs_interval,pct_days_svd,hrs) %>%
  ungroup() %>%
...
wsa_elig_dt <- wsa %>% select(Case_ID,Eligibility_Date)
...
min_waitlist_svs <-
  ...
  left_join(wsa_elig_dt, by = "Case_ID") %>%
  ...
  select(Case_ID,month,cm,cls,respite,med_rvw,fam_svs) %>%
  mutate(
    min_svs = cm >= 1 & cls >= 1 & respite >= 1 & med_rvw >= 1 & fam_svs >= 1
  ) %>%
  arrange(Case_ID, month)
```
